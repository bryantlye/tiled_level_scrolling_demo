...
game.getResourceManager().loadScene(DESERT_SCENE_PATH, 
    ...
    for (let i = 0; i < 100; i++) {
        ...
        if (i >= 50) {
            type = game.getResourceManager().getAnimatedSpriteType("PRAYING_MANTIS");
            randomSprite = new AnimatedSprite(type, "WALK");
        }
    ...
    let playerSprite : AnimatedSprite = new AnimatedSprite(type, "WALKING1");

    let sceneGraph : SceneGraph = game.getSceneGraph();

    let startX : number = sceneGraph.getViewport().getWidth()/2
    let startY : number = sceneGraph.getViewport().getHeight()/2
